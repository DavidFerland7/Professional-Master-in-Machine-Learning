...
with open('word_to_id_' + '.pickle', 'wb') as f:
    dump(word_to_id, f)
...
def run_epoch(model, data, is_train=False, lr=1.0):
    ...
    for step, (x, y) in enumerate(ptb_iterator(data, model.batch_size, model.seq_len)):
        ...
        if args.model == 'TRANSFORMER':
            ...
            outputs, hidden, states = model(inputs, hidden)
        ...
        #loss = loss_fn(outputs[-1].view(1,-1), tt[-1].view(1))

        loss = loss_fn(outputs[-1].view(128, -1), targets[-1].view(128))
        ...
        if is_train:  # Only update parameters if training
            ...
            for i, h in enumerate(states):
                ...
                if HIDDEN_LAYER == "NORM":
                    grad_norm = np.sqrt(h.grad.transpose(0, 1).contiguous().view(128, -1).mean(0).pow(2).sum().tolist())
                elif HIDDEN_LAYER == "AVG":
                    grad_norm = np.sqrt(h.grad.mean((0, 1)).pow(2).sum().tolist())

                gradients_output.append(grad_norm)
        break

    with open('gradient_' + MODEL + '_' + HIDDEN_LAYER + '.pickle', 'wb') as f:
        dump(gradients_output, f)
