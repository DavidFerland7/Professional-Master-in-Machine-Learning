...
def run_train(config):
    ...
    model, best_valid_bleu, start_epoch = get_model(
        config, model_name, [emb_en_obj, emb_fr_obj], config["model"]["model_kwargs"],
    )
    ...
    for epoch in tqdm(
        ...
        for step, (x_batch_train, y_batch_train) in tqdm(
            ...
            with tf.GradientTape() as tape:
                ...
                logits = model.decoder(y_batch_train, enc_hidden, training=True)
        ...
        left_trim = (
            "<BOS>"
            if emb_fr_obj.word_to_idx.get("<BOS>") in true_train_sequences[0]
            else None
        )
        right_trim = (
            "<EOS>"
            if emb_fr_obj.word_to_idx.get("<EOS>") in true_train_sequences[0]
            else None
        )
