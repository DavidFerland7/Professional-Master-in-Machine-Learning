...
def run_predict(config, run_name):
    ...
    ################# PREPARE DATASETS ###################
    y_true_tensor = tf.ragged.constant(df_fr_valid.text.values.tolist())
    dataset = (
        tf.data.Dataset.from_tensor_slices((x_valid, y_valid, y_true_tensor))
        .batch(32)
    ...
    ################# VALID ###################
    for step, (x_batch, y_batch, y_true) in tqdm(
        enumerate(dataset),
        total=int(num_obs_valid / config["model"]["batch_size_valid"]),
        desc="Prediction on batch",
    ):
        # Persist the X and y_true for stats
        x_sequences.extend(tf.cast(x_batch, dtype=tf.int32).to_list())
        y_sequences.extend(tf.cast(y_batch, dtype=tf.int32).to_list())

        true_sequences.extend([x.decode("utf-8") for x in y_true.numpy().tolist()])

        # Padding batch of sequences
        x_batch, y_batch = pad_batch_sequences(
            x_batch, y_batch, training=False, factor_pad=4
        )

        x_batch = tf.squeeze(x_batch)

        logits = model.evaluate(x_batch)
        predictions = tf.math.argmax(tf.nn.softmax(logits), axis=-1)

        # Predictions
        pred_sequences.extend(predictions.numpy().tolist())
    left_trim = "<BOS>" if "<BOS>" in true_sequences[0] else None
    right_trim = "<EOS>" if "<EOS>" in true_sequences[0] else None
    ...
    y_sentences = emb_fr_obj.convert_idx_to_word(
        y_sequences, left_trim=left_trim, right_trim=right_trim,
    )
    ...
    df = pd.DataFrame(
        {
            ...
            "y_pred": pred_sentences,
            "y_target": y_sentences,
            "y_true_raw": true_sentences,
            "Bleu": bleu,
        }
    )
