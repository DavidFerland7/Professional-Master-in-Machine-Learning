############## DISCLAIMER #############
# This model is an adaptation of the model found in
...
class Transformer_OneHot(BaseModel):
    def __init__(
        self,
        emb_obj_en,
        emb_obj_fr,
        ...
        super(Transformer_OneHot, self).__init__()
        ...
        self.source_vocab_size = emb_obj_en.vocab_size
        self.target_vocab_size = emb_obj_fr.vocab_size
        self.target_word_to_idx = emb_obj_fr.word_to_idx

        self.encoder_embedding_layer = Embeddinglayer(self.source_vocab_size, d_model)
        self.encoder_embedding_dropout = tf.keras.layers.Dropout(dropout_prob)
        self.decoder_embedding_layer = Embeddinglayer(self.target_vocab_size, d_model)
    ...
    def call(
        ...
        for i in range(self.encoder_count):
            encoder_tensor, _ = self.encoder_layers[i](
                encoder_tensor, inputs_padding_mask, training=training
            )
    ...
    def evaluate(self, x):
        ...
        logits = np.zeros(shape=(x.shape[0], MAX_LENGTH, self.target_vocab_size))
        logits[
            :, 0, self.target_word_to_idx["<BOS>"]
        ] = 1  # we force logits for first token to be BOS since here we input BOS to the model

        decoder_input = tf.constant(
            value=self.target_word_to_idx["<BOS>"], shape=(x.shape[0], 1)
        )
        ...
        for i in range(1, x.shape[1]):
            ...
            ) = Mask.create_masks(x, decoder_input)
            pred = self.call(
                inputs=x,
            ...
            logits[:, i, :] = tf.squeeze(pred.numpy()[:, -1, :])
...
class DecoderLayer(tf.keras.layers.Layer):
    ...
    def call(
        self, decoder_inputs, encoder_output, look_ahead_mask, padding_mask, training
    ):
