...
import os
from sklearn.metrics import classification_report, plot_confusion_matrix, accuracy_score, precision_recall_fscore_support
...
model_selected = 'LR'  # RF | NB | LR
metrics = ['accuracy']
scoring_train = metrics
scoring_tuning = metrics
scoring_test = metrics
cv_results_all = []
# scoring_tuning = ['accuracy']
# scoring_test = ['accuracy']
n_jobs= 6
...
class FeatureExtractor(BaseEstimator, TransformerMixin):
    ...
    def transform(self, df, y=None):
        ...
        df['number_days_before_appointment'] = (df['AppointmentDay'].dt.date - df['ScheduledDay'].dt.date).dt.days
...
if __name__ == '__main__':
    ...
    if 'train_format' in exec:
        if 'data_train' not in globals(): data_train = data_train_raw
    ...
    if 'train_fit' in exec:
        ...
        if 'train_save_model' in exec:
            dump(model, os.path.dirname(os.path.abspath(__file__)) + "/model_output/notkeep/{}.joblib".format(model_selected))
    ...
    if 'train_tuning' in exec:
        ...
        for scoring in scoring_tuning:
            ...
            #print("get_params : {}".format(model.get_params()))
            ...
            #pd.DataFrame(model.cv_results_).to_csv(os.path.dirname(os.path.abspath(__file__)) + '/results/tuning/cv_results_{}_{}_{}.csv'.format(model_selected, datetime.now().strftime('%Y_%m_%d__%H_%M_%S'), scoring), index=False)
            ...
            #save model
            if 'train_save_model' in exec:
                dump(model, os.path.dirname(os.path.abspath(__file__)) + "/model_output/notkeep/{}_{}.joblib".format(model_selected, scoring))
    ...
    if 'test_predict' in exec:
        ...
        for scoring in scoring_test:
            ...
            if 'test_load_model' in exec:
                model = load(os.path.dirname(os.path.abspath(__file__)) + "/model_output/notkeep/{}_{}.joblib".format(model_selected, scoring))
            ...
            if 'test_evaluate' in exec:
                ...
                #print("=====> global test accuracy = {}".format(accuracy_score(data_test_formatted[label_test], pred)))

                for normalize in [None]:
                    pass
                    #disp = plot_confusion_matrix(model, data_test_formatted[features_test], data_test_formatted[label_test], display_labels = ['No', 'Yes'], cmap = plt.cm.Blues, normalize=normalize)
                    #disp = plot_confusion_matrix(model, data_test_formatted[features_test], data_test_formatted[label_test], display_labels = [0, 1], cmap = plt.cm.Blues, normalize=normalize)
                    #disp.ax_.set_title('confusion matrix - normalize={}'.format(normalize))

                    #print("confusion matrix - normalize={}: {}".format(normalize, disp.confusion_matrix))
                #plt.show()
                ...
                #cv_results_test = pd.DataFrame(model.cv_results_)  #just to add index
                #cv_results_test_stat = cv_results_test.loc[cv_results_test["rank_test_" + scoring]==1,:]
                cv_results_ = pd.DataFrame(model.cv_results_).to_dict('records')  #just to add index
                #cv_results_ = pd.DataFrame(model.cv_results_)  #just to add index
                #cv_results_scoring = cv_results_.loc[cv_results_["rank_test_" + scoring]==1,:].to_dict('records')
    ...
    #header_base = ['model', 'scoring', 'params'] + [c for c in header_all if c.startswith("param_")] + ['accuracy_test', 'precision_test', 'recall_test', 'fbeta_score_test'] + ['rank_test_'+ metrics[0] , 'mean_test_' + metrics[0], 'mean_train_' + metrics[0], 'std_test_' + metrics[0], 'std_train_' + metrics[0]]
